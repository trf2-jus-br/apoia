...
export const ClientIFrameTest = (props: { baseUrl: string; callbackUrl: string }) => {
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            switch ((data as MessageWithType).type) {
                ...
                case 'get-prompts': {
                    ...
                    prompts.forEach((p: any) => { if (p.slug === 'PEDIDOS' && p.library) p.is_hidden = false })
                    prompts.forEach((p: any) => { if (p.slug === 'REFINAMENTO_DE_TEXTO' && p.library) p.is_hidden = true })
    ...
    const src = `${props.baseUrl}/auth/keycloak-iframe?redirect=/sidekick?process=50016349520244025114%26instance=primeiro-grau%26action=processo_selecionar`
