    ...
    return html.replace(/([Ee]vento)\s+(\d+)(,\s+\d+º?\s*Grau)?((?:\s*(?:,|e)?\s*[A-Z]+\d+)+)/gm, (match, eventWord, eventNumber, instance, rest) => {
        ...
        while ((labelMatch = labelRegex.exec(rest)) !== null) {
            ...
            if (foundTexto) {
                ...
                if (splited.length === 2)
                    id = splited[1]
                const link = `<span class="widgetlinkdocumento" data-idpiece="${escapeHtml(foundTexto.id)}" data-iddocumento="${escapeHtml(id)}" data-target="_blank" data-is2g="${is2g}" data-numprocesso="${escapeHtml(foundTexto.numeroDoProcesso)}" data-mimetype="${escapeHtml(documentType)}" data-uf="${escapeHtml(uf)}">${escapeHtml(foundTexto.label)}</span>`
