...
- `author` (VARCHAR 64, migration-032): autor em coluna própria — em prompts o autor fica dentro do JSON `content.author`; exibido na coluna "Autor" do spec `Library`
...
- `ChooseLibrary` lista apenas próprios + favoritados + PADRAO com slug casante (`documentsForChooseLibrary`); coluna "Origem" diferencia Meu/Favorito/Padrão; descoberta de docs PADRAO/PUBLICO de terceiros é em `/library` (tabs Principais/Não Avaliados)
